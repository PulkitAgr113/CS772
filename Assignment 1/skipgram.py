...
def run_skipgram(epoch, model, vocab_dict, batch_size, train_sents):
    train_tuples = Pentagram(train_sents)
    train_dataloader = DataLoader(train_tuples, batch_size=batch_size, shuffle=True)
    for idx,item in tqdm(enumerate(train_dataloader)):
        ...
        for out_ind in [0,1,3,4]:
            ...
            err += model.get_error(output_vecs, pred_vecs)
            if out_ind == 0:
        ...
        print(f"Mean error for batch {idx}: {err}")
